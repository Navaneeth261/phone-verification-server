...
export const verifyTokenResponse = async (req, res) => {
  res.status(200).json({
    ...
    data: { userId: req.body.userId, phoneNumber: req.body.phoneNumber, name: req.body.name },
  });
};
...
const generateVerificationCode = async (req, res, action_type) => {
  try {
    ...
    if (action_type === "register" && user && user.userId !== phoneNumber ) {
      ...
      return;
    } else if (action_type === "login" && (!user || user.userId === phoneNumber)) {
      res
        .status(200)
        .json({
          status: false,
    ...
    expirationTime.setMinutes(expirationTime.getMinutes() + VERIFICATION_CODE_EXPIRE_IN_MINS);
    ...
    try { 
      if(NODE_ENV === "development") {
        ...
        sendCodeRes = await sendVerificationCodeToPhoneNumber(phoneNumber, message);
    }      
    } catch (err) {
      sendCodeRes = {
        status: false,
...
export const verifyCode = async (req, res, action_type) => {
  try {
    ...
    } else if (user.verificationAttempts > parseInt(MAX_VERIFICATION_ATTEMPTS)) {
      res.status(200).json({
        status: false,
        message:
    ...
    else if (!user.verificationCode || user.verificationCodeExpiration < new Date()) {
      user.isVerified = false;
    ...
    res.status(200).json({
      ...
      data: { name: user.name, userId: user.userId, phoneNumber: user.phoneNumber },
      token,
    }); }
